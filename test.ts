...
const fetchLists = async () => {
  ...
  const response = await fetch(`http://localhost:3000/lists/user/${userId}`, {
    method: 'GET',
    headers: {
...
const createList = async (newListName: string) => {
  ...
  return response.json(); // Retourner la nouvelle liste si nécessaire
};
...
const CreateList = () => {
  ...
  const { mutate } = useMutation(createList, {
    onSuccess: () => {
      // Invalider la requête pour refetcher les listes après la création
  ...
  const handleCreate = () => {
    mutate(newListName);
    setNewListName(''); // Réinitialiser le champ de saisie
  };
  ...
  return (
    <div>
      ...
      />
      <button onClick={handleCreate}>Créer la liste</button>
    </div>
  );
};
...
//         setLists(listsData.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
