(async () => {
  ...
  const getXPathText = (xpath, attr = "textContent") => {
    const node = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
    return attr === "textContent" ? node?.textContent.trim() : node?.[attr] || null;
  };

  const cleanNumber = (value) => {
    if (!value) return 0;
    const cleanedValue = value.replace(/[^\d.,KM]/g, "").replace(",", ".");
    return cleanedValue.endsWith("M")
      ? Math.round(parseFloat(cleanedValue) * 1_000_000)
      : cleanedValue.endsWith("K")
      ? Math.round(parseFloat(cleanedValue) * 1_000)
      : Math.round(parseFloat(cleanedValue));
  };
  ...
  const extractedData = {
    ...
    followers: cleanNumber(getXPathText(xPaths.followers)),
    following: cleanNumber(getXPathText(xPaths.following)) || "0",
  ...
  const getStorageData = (key) =>
    new Promise((resolve, reject) =>
      chrome.storage.sync.get(key, (result) => chrome.runtime.lastError ? reject(new Error(chrome.runtime.lastError)) : resolve(result[key]))
    );

  try {
    const [userData, selectedList] = await Promise.all([getStorageData("userData"), getStorageData("selectedList")]);
    ...
    const existingProfile = await fetch(`${BASE_URL}/facebook/${encodeURIComponent(extractedData.profileUrl)}`)
    ...
    if (extractedData.name !== "None" && extractedData.followers && extractedData.following) {
      ...
      if (selectedList) {
        await fetch(`${BASE_URL}/lists/${selectedList}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ profiles: { add: [data] } })
        });
        console.log("List updated");
      } else {
        console.log("No list update");
      }

      chrome.runtime.sendMessage({ success: response.ok });
    } else {
...
//     profileImage:
//       "/html/body/div[1]/div/div[1]/div/div[3]/div/div/div[1]/div[1]/div/div/div[1]/div[2]/div/div/div/div[1]/div/a/div",
//   };
